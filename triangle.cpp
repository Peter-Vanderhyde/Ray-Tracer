...
Triangle::Triangle(const Point3D &vertex0, const Point3D &vertex1, const Point3D &vertex2,
    ...
    :Shape{material, texture, true}, vertex0{vertex0}, vertex1{vertex1}, vertex2{vertex2},
...
bool Triangle::bounding_box(AABB& output_box) const {
    output_box = AABB{
        Vector3D{fmin(fmin(vertex0.x, vertex1.x), vertex2.x) - Constants::Epsilon,
                 fmin(fmin(vertex0.y, vertex1.y), vertex2.y) - Constants::Epsilon,
                 fmin(fmin(vertex0.z, vertex1.z), vertex2.z) - Constants::Epsilon},
        Vector3D{fmax(fmax(vertex0.x, vertex1.x), vertex2.x) + Constants::Epsilon,
                 fmax(fmax(vertex0.y, vertex1.y), vertex2.y) + Constants::Epsilon,
                 fmax(fmax(vertex0.z, vertex1.z), vertex2.z) + Constants::Epsilon}};
