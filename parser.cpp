...
void Parser::parse(std::ifstream& input) {
    ...
    while (std::getline(input, line)) {
        ...
        else if (type == "triangleobj") {
            parse_triangleobj(ss);
        }
        else if (type == "plane") {
            parse_plane(ss);
        }
...
void Parser::parse_sky(std::stringstream& ss) {
    if (!(ss >> std::boolalpha >> sky)) {
        throw std::runtime_error(input_filename + " has malformed sky. Requires bool.");
    }
}
...
void Parser::parse_triangleobj(std::stringstream& ss) {
    Vector3D v0, v1, v2;
    Point2D t1, t2, t3;
    std::string material_name, texture_name;
    ss >> v0 >> v1 >> v2 >> t1 >> t2 >> t3 >> material_name >> texture_name;
    
    Material* material{get_material(material_name)};
    Texture* texture{get_texture(texture_name)};
    world.add(std::make_shared<TriangleObj>(v0, v1, v2, t1, t2, t3, material, texture));
}
...
void Parser::parse_fog(std::stringstream& ss) {
    ...
    Material* material{get_material(material_name)};
    Texture* texture{get_texture(texture_name)};
    world.add(std::make_shared<Fog>(center, dimensions, rotations, density, material, texture));
}
...
void Parser::parse_material(std::stringstream& ss) {
    ...
    bool emitting;
    ss >> name >> material_name >> std::boolalpha >> emitting;
    if (material_name == "diffuse") {
        materials[name] = std::make_shared<Diffuse>(emitting);
    }
    else if (material_name == "specular") {
        materials[name] = std::make_shared<Specular>(emitting);
    }
    else if (material_name == "metal") {
        double fuzz;
        if (ss >> fuzz) {
            materials[name] = std::make_shared<Metal>(emitting, fuzz);
        }
        else {
        ...
        double index_ratio;
        if (ss >> index_ratio) {
            materials[name] = std::make_shared<Glass>(emitting, index_ratio);
        }
        else {
        ...
        materials[name] = std::make_shared<Gloss>(emitting);
    }
...
void Parser::parse_texture(std::stringstream& ss) {
    ...
    if (texture_name == "solid") {
        ...
        if (ss >> color >> secondary >> num_of_stripes >> width_percent)
        {
            ...
            throw std::runtime_error("Malformed gradient");
        }
    }
